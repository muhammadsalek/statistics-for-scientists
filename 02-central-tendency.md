...
graph TD
    ...
    D --> D1[Skewness Relationship]
    I --> I1[Master Example: All 7 Measures]
```
...
### Example 5: All Seven Measures, One Dataset

**Dataset:** Daily ICU length of stay (days) for 12 patients

`2, 3, 3, 4, 4, 4, 5, 6, 7, 8, 9, 35`

| Measure | Value | Calculation Note |
|---------|-------|-------------------|
| **Mean** | 7.5 days | Pulled upward by the 35-day outlier |
| **Median** | 4.5 days | Average of the 6th and 7th ordered values (4, 5) |
| **Mode** | 4 days | Appears 3 times |
| **10% Trimmed Mean** | 5.1 days | Drops the lowest and highest value before averaging |
| **Geometric Mean** | 5.6 days | Appropriate since stay-length is a positive, multiplicative-feeling quantity |
| **Harmonic Mean** | 4.6 days | Dominated by the small values (2, 3, 3) |
| **Weighted Mean** (weight = 2 for ward transfers, patients 1–4) | 6.8 days | Depends entirely on which weights are chosen |

```mermaid
graph LR
    subgraph NumberLine["Where Each Measure Lands (days)"]
    direction LR
    Mode4["Mode = 4"] --- HM["Harmonic = 4.6"] --- Med["Median = 4.5"] --- TM["Trimmed = 5.1"] --- GM["Geometric = 5.6"] --- WM["Weighted = 6.8"] --- Mean["Mean = 7.5"]
    end

    style Mode4 fill:#2ecc71,color:#fff
    style Med fill:#27ae60,color:#fff
    style HM fill:#16a085,color:#fff
    style TM fill:#f39c12,color:#fff
    style GM fill:#e67e22,color:#fff
    style WM fill:#d35400,color:#fff
    style Mean fill:#e74c3c,color:#fff
```

> [!IMPORTANT]
> One outlier (35 days) drags the **mean 3 days above the median**. In a discharge report, saying "average ICU stay was 7.5 days" paints a much grimmer picture than the reality for a typical patient (4–5 days). This is exactly the pattern reviewers flag as **SD > Mean** (see the *Common Mistakes* section below).
...
<details>
<summary><b>Q1: Is the mean always wrong for skewed data?</b></summary>

Not wrong — just potentially *unrepresentative*. The mean is still a well-defined, mathematically useful number for skewed data (it's needed for totals, budgets, and many statistical models). The issue is purely one of **interpretation**: don't call it "the typical value" when the distribution is heavily skewed. Report both mean and median when possible.
</details>

<details>
<summary><b>Q2: Why does software sometimes report "no mode" or multiple modes?</b></summary>

The mode only exists when a value repeats. For continuous, high-precision measurements (e.g., blood pressure recorded to 0.1 mmHg), every value may be unique, so there is no mode. If two or more values tie for the highest frequency, the dataset is **multimodal** and some software reports all of them, while others report the smallest by convention — always check your software's documentation.
</details>

<details>
<summary><b>Q3: Can I use the geometric mean with zero or negative values?</b></summary>

No. The geometric mean requires strictly positive values because it relies on the logarithm, which is undefined at zero and for negative numbers. If your data can contain zeros (e.g., zero-inflated costs), consider adding a small constant before log-transforming, using a different transform, or reporting the median instead.
</details>

<details>
<summary><b>Q4: How many outliers can the median tolerate?</b></summary>

Up to (but not including) 50% of the data — this is called the **breakdown point**. Even if nearly half of your observations are extreme, the median stays anchored near the "middle" of the remaining data. The mean's breakdown point is technically 0%: even one sufficiently extreme value can move it arbitrarily far.
</details>

<details>
<summary><b>Q5: My reviewer asked for "median (IQR)" instead of "mean ± SD" — why?</b></summary>

This is one of the most common statistical reviewer comments in clinical and public-health journals. If your variable is skewed (commonly signaled by SD > mean for a non-negative variable, or by a visibly long tail), the mean ± SD pair can mislead readers about both the "typical" value and the spread. Median with interquartile range (IQR) is the standard, distribution-agnostic alternative — see the *Common Mistakes* and *Reviewer Perspective* sections.
</details>

<details>
<summary><b>Q6: What's the difference between a trimmed mean and a Winsorized mean?</b></summary>

A **trimmed mean** removes the most extreme α% of observations from each tail before averaging. A **Winsorized mean** instead *replaces* those extreme values with the nearest remaining value rather than discarding them, so the sample size (n) stays the same. Both trade some efficiency for robustness; trimming is more common in reporting, Winsorizing is more common in some robust-statistics pipelines.
</details>
...
| Term | Definition |
|------|-----------|
| **Breakdown point** | The smallest proportion of data that, if corrupted or made arbitrarily extreme, can move the estimator by an arbitrary amount. |
| **Central tendency** | A single value intended to summarize or represent the "middle" of a dataset. |
| **Interquartile Range (IQR)** | The distance between the 25th and 75th percentile; the spread measure typically paired with the median. |
| **Kurtosis** | A measure of how heavy the tails of a distribution are relative to a normal distribution. |
| **M-estimator** | A broad family of robust location estimators (including the trimmed mean and median) defined by minimizing a chosen loss function. |
| **Multimodal** | A distribution with more than one mode (peak in frequency). |
| **Robustness** | The degree to which a statistic resists distortion from outliers or violations of assumptions. |
| **Skewness** | A measure of the asymmetry of a distribution; positive skew has a long right tail, negative skew has a long left tail. |
| **Trimming proportion (α)** | The fraction of data removed from each tail when computing a trimmed mean. |
| **Winsorizing** | Replacing extreme values with a less extreme boundary value rather than removing them. |
...
|------|-----------|-------------------|
| *Statistical Methods* | Cochran & Snedecor | Chapter 3 — Sample Averages |
| *Data Analysis Using Regression and Multilevel/Hierarchical Models* | Gelman & Hill | Chapter 4 — Linear Regression Basics |
| *Introduction to Modern Statistics* | Cetinkaya-Rundel & Hardin | Chapter 2 — Summarizing Data |
| *Modern Statistics for the Social and Behavioral Sciences* | Wilcox | Chapter 2 — Numerical Summaries |
| *Introduction to Robust Estimation and Hypothesis Testing* | Wilcox | Chapters 2–3 — Robust Measures of Location |
...
- [R for Data Science: Chapter 5 — Exploratory Data Analysis](https://r4ds.had.co.nz)
...
3. Wilcox, R.R. (2012). *Introduction to Robust Estimation and Hypothesis Testing* (3rd ed.). Academic Press.
...
6. CONSORT Group (2010). CONSORT 2010 Statement: updated guidelines for reporting parallel group randomised trials. *BMJ*, 340, c332.
7. Vandenbroucke, J.P. et al. (2007). Strengthening the Reporting of Observational Studies in Epidemiology (STROBE): explanation and elaboration. *Annals of Internal Medicine*, 147(8), 573–577.
8. Huber, P.J. (1964). Robust estimation of a location parameter. *Annals of Mathematical Statistics*, 35(1), 73–101. *(Foundational paper for M-estimators and trimmed/Winsorized means.)*
9. Tukey, J.W. (1977). *Exploratory Data Analysis*. Addison-Wesley. *(Popularized robust summaries and the box plot.)*
10. Limpert, E., Stahel, W.A., & Abbt, M. (2001). Log-normal distributions across the sciences: keys and clues. *BioScience*, 51(5), 341–352. *(Rationale for using the geometric mean with skewed, multiplicative biological data.)*

> 🔎 **Note on this reference list:** entries are merged from the original "Key Papers" and "References" sections to avoid duplication — each source now appears once, with the fullest available citation detail.
